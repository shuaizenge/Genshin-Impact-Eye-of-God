#include "shenzhiyan.h"
#include "common.h"
#include "driver/sd_card.h"
#include "shenzhiyan_docoder.h"
#include "shenzhiyan_gui.h"
#include "sys/app_controller.h"
#include "app/media_player/DMADrawer.h"

#include <SD.h>

#define MEDIA_PLAYER_APP_NAME "SHENZHIYAN"
...
#define MJPEG_FILENAME "/240_20fps.mjpeg"
#define MJPEG_BUFFER_SIZE (240 * 240 * 2 / 4)

// 天气的持久化配置
#define MEDIA_CONFIG_PATH "/shenzhiyan.cfg"
struct MP_Config {
...
static ShenzhiyanAppRunData* run_data = NULL;
...
static bool video_start(bool create_new)
{
    ...
    //     Serial.println(F("mJpeg file not found\n"));
    // } else {
    //     Serial.println(F("mJpeg file found\n"));
    // }
    ...
    if (NULL != strstr(run_data->pfile->file_name, ".mjpeg") || NULL != strstr(run_data->pfile->file_name, ".MJPEG")) {
        ...
        run_data->player_docoder = new MjpegPlayDocoder_me(&run_data->file);
...
static int shenzhiyan_player_init(void)
{

    // 获取配置信息
    read_config(&cfg_data);
    run_data = (ShenzhiyanAppRunData*)calloc(1, sizeof(ShenzhiyanAppRunData));
    run_data->player_docoder = NULL;
...
static void shenzhiyan_player_process(AppController* sys,
    const ImuAction* act_info)
{
    ...
    if (run_data->file.available()) {
        // 播放一帧数据
        run_data->player_docoder->my_video_play_screen();
    } else {
        // 结束播放
        release_player_docoder();
        run_data->file.close();
...
static int shenzhiyan_player_exit_callback(void* param)
{
    // 结束播放
    release_player_docoder();
...
static void shenzhiyan_player_message_handle(const char* from, const char* to,
...
APP_OBJ shenzhiyan_app = { MEDIA_PLAYER_APP_NAME, &app_shenzhiyan, "",
    shenzhiyan_player_init, shenzhiyan_player_process,
    shenzhiyan_player_exit_callback, shenzhiyan_player_message_handle };